...
export class MovimentationController {
	...
	public static saveMovimentation(req: Request, res: Response): void {
		...
		new Promise<string>(async (result, reject) => {
			try {
				...
				await MovimentationValidator.validateMovimentation(req, repositoryProd);
				...
				var update_product: object = { quantity: product.quantity - movimentation.quantity };
